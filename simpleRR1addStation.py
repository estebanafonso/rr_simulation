...
class Crew_Homebase(object):
    ...
    def __init__(self, env, crew_list, name, min_sleep_time):
        ...
        for crew in crew_list:
            self.crew_store.put(crew)
        print "Initialized crews at home: %s" % self.crew_store.items
        
    def put_crew(self, crew):
    ...
    def get_crew(self):
        return self.crew_store.get()
        
    def view_crew(self):
        return self.crew_store.items
        
        
        
class Station(object):
    '''
    This class represents the railroad station.  Crews are periodically
    called into the station.  When a train passes through the station, he recrews.
    '''
    ...
    def call_crew_to_station(self,homebase):
        while True:
            ...
            crew = yield homebase.get_crew()
            self.crew_at_station.put(crew)
            print "Calling %s to station at %.2f." % (crew, env.now)
            print "Crew at station at %.2f: %s" % (env.now,self.view_crew())
    ...
    def send_crew_home(self,homebase):
        crew = yield self.get_crew()
        env.process(homebase.put_crew(crew))
...
class Train(object):
    ...
    def initialize_crew(self, homebase):
        ...
        print "Crews at home: %s." % homebase.view_crew()
    ...
    def recrew(self, station, homebase):
        ...
        print "Crew on %s before recrew: %s" % (self.name, self.view_crew())
        print "Crew at %s before recrew: %s" % (station.name, station.view_crew())
        print "Crew at %s before recrew: %s" % (homebase.name, homebase.view_crew())
        old_crew = yield self.get_crew()
        env.process(homebase.put_crew(old_crew))
        new_crew = yield station.get_crew()
...
railway_c = Railway(env, "track C", 8)
